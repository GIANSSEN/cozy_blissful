...
import {
  LogIn, Eye, EyeOff, AlertCircle, Clock, Mail, Lock,
...
const TRUST_POINTS = [
  ...
  { icon: Clock, title: 'Open 7 days a week', note: 'Book any time from 6 AM to 11 PM.' },
  { icon: Sparkles, title: 'Certified therapists', note: '30+ trained professionals at your service.' },
];
      ...
      <div className="relative w-full">
        ...
        <input id={id}
          ...
          style={{
            ...
            color: B.ink,
            padding: rightEl ? '0.65rem 2.75rem 0.65rem 2.6rem' : '0.65rem 0.9rem 0.65rem 2.6rem',
...
const Login = () => {
  ...
  // SECURITY: never accept session tokens from the URL — tokens in query
  // strings leak via browser history, referrer headers, and shared links.
  useEffect(() => {
  ...
  return (
    <div className="min-h-dvh w-full relative flex items-center justify-center px-4 sm:px-6 py-8 sm:py-10"
      ...
      <motion.div initial={{ opacity: 0, y: 16, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        <div className="hidden md:flex flex-col justify-between w-[42%] relative p-9 xl:p-10 flex-shrink-0"
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-black leading-tight tracking-tight mb-1.5" style={{ color: '#fff' }}>
              Welcome back<br />to your calm.
            </h2>
            <p className="text-sm leading-relaxed mb-8" style={{ color: 'rgba(255,255,255,0.65)' }}>
              Sign in to manage bookings, track rewards and enjoy member-only rates.
            </p>
              ...
              {TRUST_POINTS.map(t => (
                <li key={t.title} className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-9 h-9 rounded-xl shrink-0"
                    ...
                    <t.icon className="w-4 h-4" style={{ color: B.goldLight }} />
                  </span>
                  <span>
                    <span className="block text-sm font-bold mb-0.5" style={{ color: '#fff' }}>{t.title}</span>
                    <span className="block text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.6)' }}>{t.note}</span>
                  </span>
                </li>
              ))}
            </ul>
        ...
        {/* ─── Right form panel ─── */}
        <div className="flex-1 flex items-center justify-center px-5 sm:px-10 lg:px-12 py-8 sm:py-10" style={{ background: B.white }}>
          <div className="w-full max-w-[380px] mx-auto">
            ...
            <div className="flex flex-col items-center mb-6 text-center">
              <motion.div
                ...
                <img src="/cb-logo.jpg" alt="Cozy Blissful" className="w-14 h-14 rounded-2xl object-cover mb-3"
                  style={{ boxShadow: '0 8px 24px rgba(191,161,95,0.2), 0 2px 8px rgba(0,0,0,0.08)', border: `1.5px solid ${B.line}` }} />
              </motion.div>
              <h1 className="text-2xl font-black tracking-tight mb-1" style={{ color: B.ink }}>Welcome Back</h1>
              <p className="text-sm" style={{ color: B.inkSoft }}>Sign in to your Cozy Blissful account</p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {notice && (
                <motion.div key="note" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} role="status"
                  className="flex items-start gap-2.5 p-3 rounded-xl text-xs mb-4"
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <Input label="Password" id="login-password" name="password" type={showPw ? 'text' : 'password'}
                autoComplete="current-password" required maxLength={128} icon={Lock}
              ...
              <div className="flex items-center justify-between text-xs pt-0.5">
                <label htmlFor="remember-me" className="inline-flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <span className="font-medium" style={{ color: B.inkSoft }}>Remember me</span>
                </label>
                <Link to="/forgot-password"
                  className="font-semibold transition-colors hover:underline underline-offset-2 shrink-0"
                  style={{ color: B.gold }}>
                  Forgot password?
                </Link>
              </div>
