...
import { Sparkles, ArrowRight, Star, Calendar, MapPin, Clock, Shield, Menu, X, Heart, Award, Smile, Check, ChevronDown, Quote, Leaf, Users, Sparkle, UserCheck } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden selection:bg-emerald-200 selection:text-emerald-900" style={{ fontFamily: "'Inter',sans-serif", background: "#faf9f7" }}>
      ...
      {/* ── FULL-WIDTH ORIGINAL FIXED NAVBAR (Non-floating Edge-to-Edge Header) ── */}
      <motion.header
        ...
        style={{
          background: scrolled ? "rgba(4,18,12,0.96)" : "rgba(4,18,12,0.65)",
          backdropFilter: scrolled ? "blur(28px) saturate(2)" : "blur(12px)",
          WebkitBackdropFilter: scrolled ? "blur(28px) saturate(2)" : "blur(12px)",
          borderBottom: scrolled ? "1px solid rgba(191,161,95,0.18)" : "1px solid rgba(255,255,255,0.06)",
          boxShadow: scrolled ? "0 8px 48px rgba(0,0,0,0.45)" : "none",
        }}
      >
        <motion.div
          className="absolute top-0 left-0 right-0 h-[2px] origin-left"
          style={{ background: "linear-gradient(90deg,transparent,#bfa15f,#e8cc8a,#bfa15f,transparent)" }}
          ...
          <div className="flex items-center justify-between py-3.5 lg:py-4">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div className="leading-none">
                <span className="text-sm font-black tracking-wide text-white block group-hover:text-amber-200 transition-colors" style={{ fontFamily: "'Playfair Display', serif" }}>Cozy Blissful</span>
            ...
            <nav className="hidden md:flex items-center gap-1">
              {[["#story", "Our Story"], ["#services", "Services"], ["#how-it-works", "How It Works"], ["#why-us", "Why Us"], ["#testimonials", "Reviews"]].map(([href, label]) => (
                <a key={href} href={href} onClick={(e) => handleNavClick(e, href)} className="relative px-4 py-2 text-xs font-bold tracking-wide text-white/80 hover:text-white transition-colors group cursor-pointer">
                  {label}
                  <span className="absolute bottom-1 left-4 right-4 h-0.5 rounded-full origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300" style={{ background: "linear-gradient(90deg,#bfa15f,#e8cc8a)" }} />
                </a>
              ))}
            </nav>
            ...
            <div className="hidden md:flex items-center gap-3">
              <Link to="/login" className="px-5 py-2 text-xs font-bold rounded-xl hover:bg-white/10 transition-all text-white/85" style={{ border: "1px solid rgba(255,255,255,0.15)" }}>Log In</Link>
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                <Link to="/register" id="nav-book" className="px-5 py-2.5 text-xs font-black text-[#041e16] rounded-xl flex items-center gap-1.5" style={{ background: "linear-gradient(135deg,#bfa15f,#e8cc8a)", boxShadow: "0 4px 18px rgba(191,161,95,0.45)" }}>
            ...
            <motion.button className="md:hidden flex items-center justify-center w-10 h-10 rounded-xl" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", color: "white" }} onClick={() => setMobile(v => !v)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.93 }}>
              <AnimatePresence mode="wait">
                {mobile ? <motion.div key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.2 }}><X className="w-4 h-4 text-amber-300" /></motion.div> : <motion.div key="m" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.2 }}><Menu className="w-4 h-4" /></motion.div>}
              </AnimatePresence>
            </motion.button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {mobile && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.35, ease: EASE }} className="md:hidden overflow-hidden w-full border-t" style={{ background: "rgba(4,14,10,0.98)", backdropFilter: "blur(24px)", borderColor: "rgba(191,161,95,0.18)" }}>
              <div className="px-6 py-6 flex flex-col gap-1">
                {[["#story", "Our Story"], ["#services", "Services"], ["#how-it-works", "How It Works"], ["#why-us", "Why Us"], ["#testimonials", "Reviews"]].map(([href, label], i) => (
                  <motion.a key={href} href={href} onClick={(e) => handleNavClick(e, href)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-white/75 hover:text-white cursor-pointer" whileHover={{ backgroundColor: "rgba(191,161,95,0.08)", x: 4 }} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 + 0.08 }}>
                ...
                <div className="flex gap-2.5">
                  ...
                  <Link to="/register" onClick={() => setMobile(false)} className="flex-1 text-center py-3 rounded-xl text-sm font-bold text-[#041e16] flex items-center justify-center gap-2" style={{ background: "linear-gradient(135deg,#bfa15f,#d4b87a)", boxShadow: "0 4px 16px rgba(191,161,95,0.35)" }}><Sparkles className="w-3.5 h-3.5" />Book Now</Link>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden" style={{ background: "linear-gradient(135deg,#041e16 0%,#073328 50%,#0e4d38 100%)" }}>
        <motion.div style={{ y: hBY }} className="absolute inset-0 pointer-events-none">
          <img src="https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&w=1800&q=55" alt="Luxury Spa Setup" className="w-full h-full object-cover opacity-[0.14]" />
        </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 w-full pt-32 pb-24">
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
            <motion.div style={{ y: hTY, opacity: hOP }} className="flex-1 z-10 w-full lg:max-w-xl text-center lg:text-left">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[10px] font-bold text-amber-300 mb-6" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(191,161,95,0.28)", backdropFilter: "blur(10px)" }}>
                <Sparkles className="w-3.5 h-3.5" /><span>Cozy Blissful Salon & Spa · Premium Wellness Destination</span>
              </motion.div>
              <motion.h1 initial={{ opacity: 0, y: 48 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, delay: 0.2, ease: EASE }} className="text-4xl sm:text-5xl lg:text-[3.6rem] xl:text-[4.2rem] font-black text-white leading-[1.03] mb-6" style={{ fontFamily: "'Playfair Display', serif", letterSpacing: "-0.01em" }}>
                Step Into<br /><span style={{ WebkitTextFillColor: "transparent", WebkitBackgroundClip: "text", backgroundImage: "linear-gradient(135deg,#e8cc8a 0%,#bfa15f 55%,#d4b87a 100%)", backgroundClip: "text", fontStyle: "italic" }}>Your Sanctuary.</span>
              </motion.h1>
              <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.35 }} className="text-emerald-100/70 text-sm sm:text-base lg:text-lg mb-9 leading-relaxed max-w-md mx-auto lg:mx-0">
              ...
              <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }} className="flex flex-col sm:flex-row gap-3.5 justify-center lg:justify-start">
                <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                  <Link to="/register" id="hero-explore" className="group inline-flex items-center justify-center gap-2.5 py-4 px-8 text-sm font-black rounded-2xl" style={{ background: "linear-gradient(135deg,#c9a851,#e8cc8a)", color: "#041e16", boxShadow: "0 10px 34px rgba(191,161,95,0.44)" }}>
                    Explore Services <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </motion.div>
                <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                  <Link to="/register" id="hero-book" className="inline-flex items-center justify-center py-4 px-8 text-sm font-bold text-white/85 rounded-2xl hover:bg-white/10 transition-all" style={{ border: "1px solid rgba(255,255,255,0.2)", backdropFilter: "blur(8px)" }}>
                    Book a Session
                  </Link>
                </motion.div>
              </motion.div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.85 }} className="flex items-center gap-5 mt-9 flex-wrap justify-center lg:justify-start">
            ...
            {/* Pro Hero Spa/Salon Image Showcase */}
            <motion.div initial={{ opacity: 0, scale: 0.88 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.3, delay: 0.25, ease: EASE }} style={{ y: hIY }} className="hidden lg:flex flex-1 justify-center relative z-10">
              <div className="relative w-[440px] xl:w-[480px]">
                ...
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.9, duration: 0.7 }}
                  ...
                  style={{ background: "rgba(4,18,12,0.88)", border: "1px solid rgba(191,161,95,0.3)", boxShadow: "0 16px 40px rgba(0,0,0,0.4)" }}>
      ...
      {/* ── OUR STORY (ENHANCED PREMIUM UI/UX WITH VISUAL COMPOSITION & MOTION) ── */}
      <section id="story" className="py-24 md:py-32 px-4 sm:px-6 relative overflow-hidden" style={{ background: "linear-gradient(180deg,#faf9f7 0%,#f5f0e6 100%)" }}>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <Reveal className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-800 leading-tight" style={{ fontFamily: "'Playfair Display', serif", letterSpacing: "-0.01em" }}>
              ...
              <span style={{ WebkitTextFillColor: "transparent", WebkitBackgroundClip: "text", backgroundImage: "linear-gradient(135deg,#0a3d30,#1e5e4b)", fontStyle: "italic" }}>
                True Wellness
              </span>
            </h2>
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center mb-20">
            ...
            <Reveal dir="left" className="lg:col-span-7">
              <div className="relative">
                {/* Main Luxury Spa Photo */}
                <motion.div whileHover={{ scale: 1.01 }} transition={{ duration: 0.4 }}
                  className="relative rounded-[36px] overflow-hidden shadow-2xl border"
                  style={{ border: "2px solid rgba(191,161,95,0.35)", boxShadow: "0 28px 70px rgba(0,0,0,0.14)" }}>
                  ...
                  <div className="absolute bottom-6 left-6 right-6 text-white text-left">
                    ...
                    <h3 className="text-xl sm:text-2xl font-black text-white" style={{ fontFamily: "'Playfair Display', serif" }}>The Art of Salon Wellness</h3>
                    <p className="text-emerald-200/80 text-xs sm:text-sm mt-1">Combining traditional Hilot with modern salon pampering in our private suites.</p>
                  </div>
                </motion.div>


                {/* Floating Glassmorphic Quote Badge */}
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.7 }}
                  className="mt-4 sm:mt-0 sm:absolute sm:-top-6 sm:left-4 max-w-xs p-5 rounded-2xl backdrop-blur-xl shadow-2xl z-20"
            ...
            {/* Right Story Narrative (5 columns) */}
            <Reveal dir="right" className="lg:col-span-5" delay={0.15}>
              <div className="space-y-6 text-left">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
                    <p className="text-2xl font-black text-emerald-900">2,500+</p>
                    <p className="text-xs text-slate-500 font-semibold mt-0.5">Satisfied Guests</p>
                  </div>
                  <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
                    <p className="text-2xl font-black text-amber-700">30+</p>
                ...
                <div className="pt-2">
                  <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} className="inline-block">
                    <Link to="/register" className="inline-flex items-center gap-2 py-3.5 px-7 rounded-xl font-black text-xs text-[#041e16]"
          ...
          <Reveal delay={0.25}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                ...
                <motion.div key={title} whileHover={{ y: -6, transition: { duration: 0.25 } }}
                  ...
                  <div className="w-11 h-11 rounded-2xl flex items-center justify-center mb-4" style={{ background: `${color}14`, border: `1px solid ${color}22` }}>
      ...
      <section className="relative h-[280px] sm:h-[340px] md:h-[420px] overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-6">
          <Reveal>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-white leading-tight max-w-2xl mx-auto" style={{ fontFamily: "'Playfair Display', serif" }}>"Your Serenity is Our<br /><span style={{ color: "#d4b87a", fontStyle: "italic" }}>Greatest Craft."</span></h2>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <Reveal delay={0.1} className="flex overflow-x-auto no-scrollbar py-2 px-1 justify-start sm:justify-center gap-2.5 sm:gap-3.5 mb-10 flex-nowrap sm:flex-wrap">
            {Object.entries(SVCS).map(([key, c]) => (
              <motion.button key={key} onClick={() => { setTab(key); setShowAll(false); }} className="flex-shrink-0 flex items-center gap-2 px-5 py-3 rounded-2xl text-xs font-bold transition-all duration-300"
      ...
      <section id="why-us" className="py-20 md:py-28 px-4 sm:px-6 relative overflow-hidden" style={{ background: "linear-gradient(180deg,#f2ebe0,#faf9f7)" }}>
        ...
        <div className="max-w-6xl mx-auto relative">
          <Reveal className="text-center mb-14">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-800 leading-tight" style={{ fontFamily: "'Playfair Display', serif" }}>Why Cozy Blissful?</h2>
            <p className="text-slate-400 max-w-sm mx-auto mt-3 text-sm leading-relaxed">The highest standards of safety, quality, and luxury in salon & spa wellness.</p>
          </Reveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-6">
            {[
              { icon: Heart, title: "Tailored Treatments", desc: "Every session adapts to your body, pressure preference, and target areas for maximum benefit." },
              { icon: Award, title: "Certified Therapists", desc: "All specialists are clinically trained, background-checked, and assigned by our admin team." },
              { icon: Clock, title: "Flexible Schedule", desc: "Open 7 days a week, 9 AM to 9 PM — book ahead or walk in anytime." },
              { icon: MapPin, title: "Private Salon Suites", desc: "Enjoy treatments in our serene, dedicated salon suites designed for total relaxation." },
              { icon: Shield, title: "Hygienic & Verified", desc: "Hospital-grade sanitation standards and premium tools — refreshed between every client." },
              { icon: Smile, title: "Loyalty Rewards", desc: "Earn stamps per completed session and redeem them for free treatments anytime." },
            ].map((f, i) => {
              const Icon = f.icon;
              return (
                <Reveal key={f.title} delay={i * 0.08}>
                  <motion.div whileHover={{ y: -10, transition: { duration: 0.26 } }} className="p-7 rounded-3xl group cursor-default" style={{ background: "linear-gradient(145deg,#fdfdfc,#faf8f5)", boxShadow: "14px 14px 32px #e6dfd5,-4px -4px 12px #fff", border: "1px solid rgba(255,255,255,0.92)" }}>
      ...
      <section className="py-20 md:py-28 px-4 sm:px-6 relative overflow-hidden" style={{ background: "linear-gradient(135deg,#041e16,#073328,#0e4d38)" }}>
        ...
        <Reveal className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white leading-tight mb-5" style={{ fontFamily: "'Playfair Display', serif" }}>Your Sanctuary Awaits.<br /><span style={{ color: "#d4b87a", fontStyle: "italic" }}>Book Today.</span></h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3.5 justify-center">
            <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
              <Link to="/register" id="cta-book" className="group py-4 px-9 font-bold rounded-2xl flex items-center justify-center gap-3 text-sm" style={{ background: "linear-gradient(135deg,#d4b87a,#bfa15f)", color: "#041e16", boxShadow: "0 10px 32px rgba(191,161,95,0.4)" }}>
                Book a Treatment<ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
            </motion.div>
            <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
              <Link to="/login" id="cta-signin" className="py-4 px-9 font-bold text-white/85 rounded-2xl flex items-center justify-center text-sm transition-all" style={{ border: "1px solid rgba(255,255,255,0.18)", backdropFilter: "blur(8px)" }}>Sign In</Link>
            </motion.div>
          </div>
        </Reveal>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 pt-14 pb-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3.5">
              {[["#story", "Our Story"], ["#services", "Our Services"], ["#how-it-works", "How It Works"], ["#why-us", "Why Choose Us"], ["#testimonials", "Client Reviews"]].map(([href, label]) => (
                <a key={href} href={href} className="text-white/45 text-xs font-semibold hover:text-amber-400 transition-colors flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-white/20 flex-shrink-0" />{label}</a>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-col gap-3.5">
              {[["Sign In", "/login"], ["Create Account", "/register"], ["Book a Session", "/register"]].map(([label, to]) => (
                <Link key={label} to={to} className="text-white/45 text-xs font-semibold hover:text-amber-400 transition-colors flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-white/20 flex-shrink-0" />{label}</Link>
              ))}
            </div>
          </div>
        </div>
