...
import {
  ...
  FileText, Trash2, Plus, Eye, EyeOff, Info, CalendarDays,
} from "lucide-react";
...
function TabSales(){
  ...
  const handleExport=()=>{
    exportCSV(
      ...
      filtered.map(b=>{const t=THERAPISTS.find(x=>x.id===b.tid);
...
function TabPayroll(){
  ...
  const doRelease=(rk,name,amount)=>setConfirm({rk,title:"Release Weekly Salary",
...
function TabRemittance(){
  ...
  const doMarkRemitted=(rk,name,gross)=>setConfirm({rk,
    ...
    <div className="p-4 rounded-2xl" style={{background:C.card,boxShadow:C.sh}}>
      ...
      <div className="w-full h-2 rounded-full overflow-hidden" style={{background:C.inner}}>
        <div className="h-full rounded-full transition-all duration-500"
          style={{width:`${allCount?Math.round((remittedCount/allCount)*100):0}%`,
                ...
                <div className="px-3 sm:px-4 pb-4 space-y-3" style={{borderTop:`1px solid ${C.div}`,paddingTop:14}}>
                  {Object.entries(tMap).map(([tidStr,data])=>{
                    ...
                    return(<div key={tidStr} className="rounded-2xl overflow-hidden border"
                      style={{borderColor:isR?"rgba(5,150,105,0.25)":"rgba(255,255,255,0.06)",
...
function TabExpenses(){
  ...
  const is=(err)=>({background:C.ibg,border:`1.5px solid ${err?'#f87171':C.ibdr}`,color:C.txt,
    boxShadow:err?"0 0 0 3px rgba(248,113,113,0.1)":"none"});

  const cancelForm=()=>{setAdding(false);setEditItem(null);setErrors({});
    setForm({desc:"",cat:"Supplies",amt:"",date:new Date().toISOString().slice(0,10),notes:"",status:"pending"});};
    ...
    <AnimatePresence>
      {(adding||editItem)&&(
        <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}
          className="p-5 rounded-2xl space-y-4" style={{background:C.card,boxShadow:C.sh,
