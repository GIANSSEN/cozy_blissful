...
import {
  ...
  ChevronRight, Award, Edit3, MessageCircle,
...
const getAvatarBg = (name = 'U') => {
  ...
  return `hsl(${hues[(name.charCodeAt(0) || 65) % hues.length]}, 65%, 45%)`;
};
...
const StarRating = ({ rating }) => (
  <div className="flex items-center gap-0.5" role="img" aria-label={`${rating} out of 5 stars`}>
    {Array.from({ length: 5 }).map((_, i) => (
      <Star key={i} className={`w-3.5 h-3.5 ${i < rating ? 'text-amber-400 fill-amber-400' : 'text-slate-300 dark:text-slate-700'}`} aria-hidden="true" />
    ))}
  </div>
);
...
/*  MODAL BACKDROP                                                     */
/* ------------------------------------------------------------------ */
const ModalBackdrop = ({ onClose, children, labelId }) => {
...
/*  CUSTOMER CARD                                                      */
/* ------------------------------------------------------------------ */
const CustomerCard = ({ customer: c, onViewProfile, C, idx, isDark }) => (
  <motion.article
    initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
    transition={{ delay: idx * 0.04, duration: 0.25 }}
    className="p-5 rounded-3xl border flex flex-col justify-between transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
    ...
    <div>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <div
            className="w-11 h-11 rounded-2xl flex items-center justify-center font-black text-white text-base flex-shrink-0 shadow-sm"
            style={{ background: getAvatarBg(c.name) }} aria-hidden="true"
          >{c.name.charAt(0).toUpperCase()}</div>
          <div className="min-w-0">
            <h3 className="font-extrabold text-sm leading-snug truncate" style={{ color: C.textPrimary }} title={c.name}>{c.name}</h3>
        ...
        {c.tier === 'VIP' ? (
          <span className="text-[10px] font-black px-2.5 py-1 rounded-full border flex items-center gap-1 flex-shrink-0"
      ...
      <div className="space-y-2 text-xs font-semibold" style={{ color: C.textSecondary }}>
        <div className="flex items-center gap-2 min-w-0">
          <Mail className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" aria-hidden="true" />
          <a href={`mailto:${c.email}`} className="truncate hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors" title={c.email}>{c.email}</a>
        </div>
        {c.phone && (
          <div className="flex items-center gap-2">
            <Phone className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" aria-hidden="true" />
            <a href={`tel:${c.phone}`} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">{c.phone}</a>
          </div>
        )}
      </div>

      {c.notes && (
        <div className="mt-3 p-3 rounded-2xl text-[11px] font-medium leading-relaxed"
          style={{ background: isDark ? 'rgba(15,20,32,0.6)' : '#f8fafc', color: C.textMuted, borderLeft: '2px solid #059669' }}>
          <p className="line-clamp-2"><strong className="text-emerald-600 dark:text-emerald-400">Notes: </strong>{c.notes}</p>
        </div>
      )}
    </div>

    <div className="mt-4 pt-3 border-t flex items-center justify-between gap-3" style={{ borderColor: C.cardBorder }}>
      ...
      <button onClick={onViewProfile}
        className="px-3 py-1.5 rounded-xl text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 active:scale-95 transition-all flex items-center gap-1 flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
...
const AdminCustomers = () => {
  ...
  const handleAddCustomerSubmit = async (data) => {
    try {
      ...
      showToastMsg(`Customer "${res.data.customer.name}" registered successfully`);
    } catch (err) {
  ...
  const handleUpdateNotes = async (customerId, updatedNotes) => {
    try {
      ...
      showToastMsg('Customer notes updated successfully!');
    } catch (err) {
      showToastMsg('Failed to update notes.', 'error');
    }
  };
  ...
  return (
    <AdminLayout title="Customers" subtitle="Unified Client Registry & Session History" icon={Users}>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed top-4 right-4 left-4 sm:left-auto z-[60] sm:min-w-[280px] sm:max-w-sm px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-3 border text-xs font-bold"
            style={{
              background: toast.type === 'error' ? '#7c2d12' : '#064e3b',
              color: '#ffffff', borderColor: 'rgba(255,255,255,0.15)'
            }}
          >
            <CheckCircle2 className="w-4 h-4 text-emerald-300 flex-shrink-0" aria-hidden="true" />
          ...
          {KPI.map((m, i) => (
            <motion.div key={m.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-end justify-between gap-2">
                <span className="text-xl sm:text-2xl font-black leading-none" style={{ color: C.textPrimary }}>{m.value}</span>
        ...
        <div className="p-3 sm:p-4 rounded-2xl sm:rounded-3xl border" style={{ background: C.cardBg, borderColor: C.cardBorder }}>
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-extrabold" style={{ color: C.textPrimary }}>Client Registry</h2>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setShowAddModal(true)}
                  className="px-3 sm:px-4 py-2 rounded-xl text-xs font-black text-white shadow-md hover:opacity-90 active:scale-95 transition-all flex items-center gap-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input type="search"
                  ...
                  className="w-full pl-9 pr-9 py-2.5 text-xs rounded-xl border outline-none font-medium transition-all focus:ring-2 focus:ring-emerald-500/30"
                  style={{ background: C.inputBg, borderColor: searchQuery ? '#059669' : C.inputBorder, color: C.textPrimary }} />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} aria-label="Clear search"
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center hover:bg-slate-300 dark:hover:bg-slate-600 transition-all">
              ...
              <select value={tierFilter} onChange={e => setTierFilter(e.target.value)} aria-label="Filter by tier"
                className="px-3 py-2.5 text-xs rounded-xl border outline-none font-bold cursor-pointer transition-all focus:ring-2 focus:ring-emerald-500/30 flex-shrink-0"
        ...
        <AnimatePresence mode="wait">
          ...
          {!loading && !fetchError && customers.length === 0 && (
            <motion.div key="empty-all" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              className="py-20 text-center border rounded-3xl" style={{ background: C.cardBg, borderColor: C.cardBorder }}>
          ...
          {!loading && !fetchError && filteredCustomers.length > 0 && (
            <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              {filteredCustomers.map((cust, idx) => (
                <CustomerCard key={cust.id} customer={cust} onViewProfile={() => setSelectedCustomer(cust)} C={C} idx={idx} isDark={isDark} />
              ))}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* MODALS */}
      <AnimatePresence>
        {showAddModal && <AddCustomerModal key="add" onClose={() => setShowAddModal(false)} onSubmit={handleAddCustomerSubmit} theme={theme} />}
      </AnimatePresence>
      <AnimatePresence>
        {selectedCustomer && <CustomerDetailModal key="detail" customer={selectedCustomer} onClose={() => setSelectedCustomer(null)} onSaveNotes={handleUpdateNotes} theme={theme} />}
      </AnimatePresence>
...
const AddCustomerModal = ({ onClose, onSubmit, theme }) => {
  ...
  return (
    <ModalBackdrop onClose={onClose} labelId="add-title">
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        style={{ background: Cs.bg, borderColor: Cs.border, maxHeight: '95dvh' }}>

        <div className="px-5 sm:px-6 py-4 bg-gradient-to-r from-emerald-950 to-slate-900 text-white flex items-center justify-between flex-shrink-0">
          <div>
          ...
          <button onClick={onClose} aria-label="Close"
            className="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">
            <X className="w-4 h-4" aria-hidden="true" />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4" noValidate>
          ...
          <div>
            ...
            {errors.name && <p role="alert" className="text-[10px] font-bold text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" aria-hidden="true" />{errors.name}</p>}
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="f-email" className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400 block mb-1.5">Email Address <span className="text-red-500" aria-hidden="true">*</span></label>
              ...
              {errors.email && <p role="alert" className="text-[10px] font-bold text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" aria-hidden="true" />{errors.email}</p>}
            </div>
            <div>
              <label htmlFor="f-phone" className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400 block mb-1.5">Phone Number <span className="text-slate-400 normal-case font-normal">(Optional)</span></label>
              ...
              {errors.phone && <p role="alert" className="text-[10px] font-bold text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" aria-hidden="true" />{errors.phone}</p>}
            </div>
          </div>
          ...
          <div>
            <label htmlFor="f-notes" className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400 block mb-1.5">
              Treatment Preferences & Notes <span className="text-slate-400 normal-case font-normal">(Optional)</span>
            </label>
          ...
          <div className="pt-2 flex items-center justify-end gap-3 border-t" style={{ borderColor: Cs.rowBorder }}>
            <button type="button" onClick={onClose}
              className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">Cancel</button>
            <button type="submit" disabled={submitting}
              className="px-5 py-2.5 rounded-xl text-xs font-black text-white bg-emerald-600 hover:bg-emerald-700 shadow-md active:scale-95 transition-all flex items-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">
...
const CustomerDetailModal = ({ customer, onClose, onSaveNotes, theme }) => {
  ...
  return (
    <ModalBackdrop onClose={onClose} labelId="detail-title">
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 28 }}
        className="w-full sm:max-w-2xl rounded-t-3xl sm:rounded-3xl border shadow-2xl overflow-hidden flex flex-col"
        style={{ background: Cs.bg, borderColor: Cs.border, maxHeight: '95dvh' }}>

        {/* Header */}
        <div className="p-5 sm:p-6 bg-gradient-to-r from-emerald-950 to-slate-900 text-white relative flex-shrink-0">
          <button onClick={onClose} aria-label="Close"
            className="absolute top-4 right-4 sm:top-5 sm:right-5 w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">
            <X className="w-4 h-4" aria-hidden="true" />
          </button>
          <div className="flex items-center gap-4 pr-10">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center font-black text-white text-xl shadow-lg border-2 border-white/20 flex-shrink-0"
              style={{ background: getAvatarBg(customer.name) }} aria-hidden="true">
              {customer.name.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 id="detail-title" className="text-lg sm:text-xl font-black text-white truncate">{customer.name}</h2>
                {customer.tier === 'VIP' && (
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-emerald-200/70">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Est. Spent', value: formatCurrency(customer.totalSpent), cls: 'text-emerald-600 dark:text-emerald-400 text-base font-black break-all' },
          ...
          <section aria-labelledby="pref-label">
            <div className="flex items-center justify-between mb-2">
              <label id="pref-label" htmlFor="edit-notes" className="text-xs font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
            ...
            <textarea id="edit-notes" rows={4} value={notes} onChange={e => onNotesChange(e.target.value)}
              className="w-full p-3 rounded-2xl border text-xs font-medium outline-none resize-none transition-all focus:ring-2 focus:ring-emerald-500/30"
          ...
          <section aria-labelledby="hist-label">
            <h3 id="hist-label" className="text-xs font-extrabold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-1.5">
            ...
            {(!customer.history || customer.history.length === 0) ? (
              <div className="py-10 text-center rounded-2xl border" style={{ background: Cs.row, borderColor: Cs.rowBorder }}>
                <Calendar className="w-8 h-8 mx-auto text-slate-300 dark:text-slate-700 mb-2" aria-hidden="true" />
                ...
                {customer.history.map(b => (
                  <div key={b.id} role="listitem"
                    ...
                    <div className="flex items-start gap-2.5 min-w-0">
                      <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${b.status === 'Completed' ? 'bg-emerald-500' : b.status === 'Cancelled' ? 'bg-red-400' : 'bg-amber-400'}`} aria-hidden="true" />
                      <div className="min-w-0">
                        <h4 className="font-extrabold truncate" style={{ color: Cs.text }}>{b.service}</h4>
                        <p className="text-[11px] text-slate-400 mt-0.5">{b.date} · <span className="font-medium">Therapist:</span> {b.therapist}</p>
                      </div>
                    </div>
